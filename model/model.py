...
class IM2TEXT(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        for layer in self.layers:
            x = layer(x)
        # pdb.set_trace()
        return self.fc_out(x) # [B, 768]

# class IM2MultiTEXT(nn.Module):
#     def __init__(self, embed_dim=512, middle_dim=512, output_dim=512, n_layer=2, dropout=0.1):
#         super().__init__()
#         self.fc_out1 = nn.Linear(middle_dim, output_dim)
#         self.fc_out2 = nn.Linear(middle_dim, output_dim)
#         self.fc_out3 = nn.Linear(middle_dim, output_dim)
#         layers = []
#         dim = embed_dim
#         for _ in range(n_layer):
#             block = []
...
#         for layer in self.layers:
#             x = layer(x)
#         x1 = self.fc_out1(x).unsqueeze(1)
#         x2 = self.fc_out2(x).unsqueeze(1)
#         x3 = self.fc_out3(x).unsqueeze(1)
#         out = torch.cat((x1, x2, x3), dim=1) # [B, 3, 768]
#         # pdb.set_trace()
#         return out
...
class IM2MultiTEXT(nn.Module):
    def __init__(self, embed_dim=512, middle_dim=512, output_dim=512, n_layer=2, dropout=0.1):
        ...
        self.mlp1 = nn.Sequential(nn.Linear(embed_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, output_dim))
        self.mlp2 = nn.Sequential(nn.Linear(embed_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, output_dim))
        self.mlp3 = nn.Sequential(nn.Linear(embed_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, middle_dim), nn.Dropout(dropout), nn.ReLU(),
                                  nn.Linear(middle_dim, output_dim))

    def forward(self, x):
        x1 = self.mlp1(x).unsqueeze(1)
        x2 = self.mlp2(x).unsqueeze(1)
        x3 = self.mlp3(x).unsqueeze(1)
        out = torch.cat((x1, x2, x3), dim=1) # [B, 3, 768]
        # pdb.set_trace()
        return out
...
class PromptLearner(nn.Module):
    def __init__(self, args, clip_model):
        ...
        # pdb.set_trace()
        # self.prompt_vectors = nn.ParameterList([nn.Parameter(torch.empty(args.n_ctx, prompt_dim))
        #                                         for _ in range(args.batch_size)])
        # for p in self.prompt_vectors:
        #     nn.init.normal_(p, std=0.02)
        # self.prompt_vectors = nn.Parameter(torch.empty(args.batch_size, args.n_ctx, prompt_dim))

        if args.meta_prompt and args.n_ctx > 0:
            # self.meta_net = nn.Sequential(nn.Linear(prompt_dim, prompt_dim // 16),
            #                               nn.ReLU(inplace=True),
            #                               nn.Linear(prompt_dim // 16, prompt_dim))
            # 20240223
            # self.meta_net = nn.Sequential(nn.Linear(prompt_dim, prompt_dim * 2), nn.ReLU(), nn.Dropout(0.5),
            #                               nn.Linear(prompt_dim * 2, prompt_dim * 2), nn.ReLU(), nn.Dropout(0.5),
            #                               nn.Linear(prompt_dim * 2, prompt_dim), nn.ReLU(), nn.Dropout(0.5))
            # 20240224
    ...
    # ****************************************************************    
        ...
        self.register_buffer("embedding_suffix", text_embedding[:, 1+args.n_ctx+args.n_img:, :]) # EOS, CLS

        # symble = tokenize(text)
        # symble = symble.cuda(args.gpu, non_blocking=True)
        # symble = symble.view(1, -1)
        # symble_embedding = clip_model.token_embedding(symble).type(dtype)
        # # pdb.set_trace()
        # self.register_buffer("token_prefix", symble_embedding[:, :1, :]) # SOS
        # self.register_buffer("token_suffix", symble_embedding[:, 1:, :]) # EOS, CLS
    ...
    def forward(self, token_features):
        ...
        if self.args.meta_prompt:
            ...
            # pdb.set_trace()
            text_embedding = self.construct_prompts(prefix, suffix, context, token_features)
            # pdb.set_trace()
        else:
            text_embedding = self.construct_prompts(prefix, suffix, self.prompt_vectors, token_features)
        # pdb.set_trace()
        return text_embedding

    def construct_prompts(self, prefix, suffix, prompt_vectors, token_features):
        bsz = token_features.shape[0]
        # n_ctx = prompt_vectors.shape[1]
        if token_features.dim() == 2:
            token_features = token_features.reshape(bsz, 1, -1)
...
class PseudoOnly(nn.Module):
    ...
    def forward(self, token_features):
        prefix = self.embedding_prefix
        suffix = self.embedding_suffix
        text_embedding = self.construct_prompts(prefix, suffix, token_features)
        # pdb.set_trace()
        return text_embedding

    def construct_prompts(self, prefix, suffix, token_features):
        bsz = token_features.shape[0]
        # n_ctx = prompt_vectors.shape[1]
        if token_features.dim() == 2:
            token_features = token_features.reshape(bsz, 1, -1)
...
class MultimodalPromptLearner(nn.Module):
    def __init__(self, args, clip_model):
        ...
        self.copuond_prompts_visual = nn.ParameterList([nn.Parameter(torch.empty(n_ctx, 768))
                                                        for _ in range(self.compound_prompts_depth - 1)])
        
        for single_para in self.copuond_prompts_visual:
            nn.init.normal_(single_para, std=0.02)

        img2txt = nn.Linear(768, ctx_dim)
        self.i2t_compound_prompt_projections = _get_clones(img2txt, self.compound_prompts_depth - 1)
    ...
    def forward(self, token_features):
        ...
        text_embedding = self.construct_prompts(prefix, suffix, ctx, token_features)
        
        text_deep_prompts = []
        for index, layer in enumerate(self.i2t_compound_prompt_projections):
            text_deep_prompts.append(layer(self.copuond_prompts_visual(index)))

        shared_ctx = text_embedding[:, 1 + self.n_ctx, :]

        return text_embedding, self.proj(shared_ctx), self.copuond_prompts_visual, text_deep_prompts


    def construct_prompts(self, prefix, suffix, ctx, token_features):
        bsz = token_features.shape[0]
        # n_ctx = prompt_vectors.shape[1]
        if token_features.dim() == 2:
            token_features = token_features.reshape(bsz, 1, -1)
        if ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(bsz, -1, -1)
...
class CLIP(nn.Module):
    ...
    def encode_pseudo(self, text_token, text_embedding):
    ...
    def encode_text_img(self, text, img_tokens):
        ...
        collect_ind = text == self.end_id 
        collect_ind = collect_ind.nonzero()[:, 1]
        img_tokens = img_tokens.view(b_size, 1, -1) # [B, 1, 768]
    ...
    def encode_text_img_multiple(self, text, img_tokens):
        b_size = img_tokens.size(0)
        x = self.token_embedding(text).type(self.dtype)
        collect_ind = text == self.end_id
        collect_ind = collect_ind.nonzero()[:, 1]
        x = torch.cat([x[:, :collect_ind[0]], img_tokens, x[:, collect_ind[0]:-(img_tokens.shape[1])]], dim=1)
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)
    ...
    def encode_text_img_learnable(self, text_token, text_embedding):
        x = text_embedding + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)
    ...
    def encode_text_img_retrieval_multi(self, text, img_tokens, split_ind=4, repeat=True):
        # text.shape = [1, n_ctx]
        # img_tokens.shape = [batch_size, d_model]        
        if isinstance(img_tokens, tuple):
            b_size = img_tokens[0].shape[0]
        ...
        if isinstance(img_tokens, tuple):
            ...
            ind_insert = ind_insert.nonzero()[0]
            # pdb.set_trace()
            x = torch.cat([x[:, :ind_insert], img_tokens, x[:, ind_insert+1: 78-img_tokens.shape[1]]], dim=1)
        #x = torch.cat([x, torch.zeros_like(x).cuda()[:, :1, :]], dim=1)
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        ...
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)    
        x = x[torch.arange(x.size(0)), collect_ind+img_tokens.shape[1]-1] @ self.text_projection
        return x
    
    def encode_text_img_retrieval_learnable(self, n_ctx, n_img, text_embedding, target_caption_token):
        
        if target_caption_token.shape[0] == 1:
            bsz = text_embedding.shape[0]
            target_caption_token = target_caption_token.repeat(bsz, 1)
        x_init = self.token_embedding(target_caption_token).type(self.dtype)  # [batch_size, n_ctx, d_model]
        
        x = torch.cat([text_embedding[:, :1+n_ctx+n_img], x_init[:, 1+n_ctx+n_img:]], dim=1) # construct modification text
        # pdb.set_trace()
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        ...
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)    
        x = x[torch.arange(x.size(0)), target_caption_token.argmax(dim=-1)] @ self.text_projection
        return x
