...
from model.model import convert_weights, CLIP, IM2TEXT, IM2MultiTEXT, PromptLearner, PseudoOnly
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.pseudo_only:
        pseudo_only = PseudoOnly(args, model)
    ...
    if not torch.cuda.is_available():
        ...
        if args.precision == "fp16":
            ...
            if args.prompt_learner == 'learnable':
        ...
        if args.distributed:
            ...
            if args.prompt == 'learnable':
                prompt_learner = torch.nn.parallel.DistributedDataParallel(prompt_learner, device_ids=[args.gpu], find_unused_parameters=False)
            # if args.pseudo_only:
            #     pseudo_only = torch.nn.parallel.DistributedDataParallel(pseudo_only, device_ids=[args.gpu], find_unused_parameters=False)
        if args.dp:
    ...
    if args.train_data is None:
        ...
        if args.prompt == 'fixed':
            ...
            optimizer = optim.AdamW(
                [
                    {"params": gain_or_bias_params, "weight_decay": 0.},
                    {"params": rest_params, "weight_decay": args.wd},
                    {"params": prompt_learner.parameters(), "weight_decay": args.wd}
                ],
                lr=args.lr,
                betas=(args.beta1, args.beta2),
                eps=args.eps,
        ...
        # optimizer = optim.AdamW(
        #     [
        #         {"params": gain_or_bias_params, "weight_decay": 0., "lr": args.lr, "betas": (args.beta1, args.beta2), "eps": args.eps},
        #         {"params": rest_params, "weight_decay": args.wd, "lr": args.lr, "betas": (args.beta1, args.beta2), "eps": args.eps},
        #         {"params": prompt_learner.parameters(), "weight_decay": args.wd, "lr": args.lr}
        #     ])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.prompt == 'fixed':
            if args.pseudo_only:
                train(model, img2text, data, epoch, optimizer, scaler, scheduler, args, pseudo_only, writer)
            else:
                train(model, img2text, data, epoch, optimizer, scaler, scheduler, args, writer)
        elif args.prompt == 'learnable':
            train(model, img2text, data, epoch, optimizer, scaler, scheduler, args, prompt_learner, writer)
                        ...
                        torch.save(
                            {
                                ...
                                "state_dict_pseudo_token": pseudo_only.state_dict(),
                    ...
                    torch.save(
                        {
                            ...
                            "state_dict": model.state_dict(),
                            "state_dict_img2text": img2text.state_dict(),
                            "state_dict_prompt_learner": prompt_learner.state_dict(),
                            "optimizer": optimizer.state_dict(),
                        },
                        os.path.join(args.checkpoint_path, f"epoch_{epoch + 1}.pt"),
                    )
