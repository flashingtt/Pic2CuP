...
class CIRR(Dataset):
    def __init__(self, args, transforms, mode='caps', 
    vis_mode=False, test=False, root='./data'):
        ...
        if self.test:
            ...
            if self.mode == 'caps':
                ...
                # compared experiment visualization
                if args.visual_compared:
                    self.txt_file = args.best_visual_path
                    self.query_ref = []
                    self.target_Rat1 = []
                    with open(self.txt_file, 'r') as f:
                        lines = f.readlines()
                        for l in lines:
                            self.query_ref.append(l.split("\t")[0].replace(':', ''))
                            self.target_Rat1.append(l.split("\t")[1])
    ...
    def init_val(self, data):
        for d in data:
            if self.args.visual_compared:
                if d['reference'] in self.query_ref:
                    index = np.where(np.array(self.query_ref) == d['reference'])
                    if d['target_hard'] in np.array(self.target_Rat1)[index]:
                        ref_path = d['reference']+ ".png"
                        tar_path = d['target_hard']+ ".png"
                        members = d['img_set']['members']
                        gm = []
                        for m in members:
                            g = m + ".png"
                            gm.append(g)
                            # print('\n', gm)
                        self.group_members.append(gm)
    ...
    def return_valdata(self, idx):
        if self.mode == 'caps' and not self.vis_mode:
            ...
            ref_images = self.transforms(Image.open(img_path))
            target_cap = self.target_caps[idx]
            if self.args.prompt == 'fixed':
                text_with_blank = 'a photo of * , {}'.format(target_cap)
            elif self.args.prompt == 'learnable':
                prompt = " ".join(["X"] * self.args.n_ctx)
                img_token = " ".join(["Y"] * self.args.n_img)
                text = prompt + " " + img_token
                # print(prompt)
                text_with_blank = '{}, {}'.format(text, target_cap)
            # print(text_with_blank)
            # print(target_cap)
            caption_only = tokenize(target_cap)[0]
            ref_text_tokens = tokenize(text_with_blank)[0]
            # print(len(self.group_members))
            # print(self.group_members[idx])                 
...
def targetpad_transform(target_ratio: float, dim: int):
    ...
    :param targt_ratio: target ratio for TargetPad
...
class FashionIQ(Dataset):
    def __init__(self, args, cloth, transforms, is_train=False, vis_mode=False, \
        mode='caps', is_return_target_path=False, root='./data'):
        self.args = args
        root_iq = os.path.join(root, 'fashionIQ')
    ...
    def init_data(self):
        def load_data(data):
            for d in data:
                if self.args.visual_compared:
                    if d['candidate'] in self.query_ref:
                        index = np.where(np.array(self.query_ref) == d['candidate'])
                        if d['target'] in np.array(self.target_Rat1)[index]:
    ...
    def return_all(self, idx):
        ...
        if self.args.prompt == 'fixed':
            text_with_blank = 'a photo of * , {} and {}'.format(cap2, cap1) # load both two captions
        elif self.args.prompt == 'learnable':
            prompt = " ".join(["X"] * self.args.n_ctx)
            img_token = " ".join(["Y"] * self.args.n_img)
            text = prompt + " " + img_token
            # text_with_blank = text + " " + cap2 + " " + cap1
            # text_with_blank = 'a photo of * , {} and {}'.format(cap2, cap1)
            text_with_blank = '{} , {} and {}'.format(text, cap2, cap1)
            modified_only = '{} and {}'.format(cap2, cap1)
            modified_only_token = tokenize(modified_only)[0]
            # print(text)
        token_texts = tokenize(text_with_blank)[0]                
        if self.is_return_target_path:
            return ref_images, target_images, token_texts, modified_only_token, \
...
class CsvCOCO(Dataset):
    def __init__(self, args,  transforms, transforms_region, sep=",",
        ...
        if args.visual_compared:
            ...
            with open(self.txt_file, 'r') as f:
                lines = f.readlines()
                for l in lines:
                    self.query_ref.append(l.split("\t")[0])
                print(self.query_ref)
    ...
    def __getitem__(self, idx):
        ...
        if self.args.prompt == 'fixed':       
            ...
            img_token = " ".join(["Y"] * self.args.n_img)
            text = prompt + " " + img_token
            text_with_blank = '{} and {}'.format(text, " and ".join(other_classes))
            text_with_queryclass = '{} and {} and {}'.format(text, query_class, " and ".join(other_classes))
            text_full = '{} and {}'.format(query_class, " and ".join(other_classes))   
...
class ImageList(Dataset):
    def __init__(self, args, input_filename, mod, transforms, root=None, 
    ...
    def __getitem__(self, idx):
        if self.mod == 'target':
            ...
            if self.return_filename:
                ...
                return images, target, img_path       
            else:
                return images
        else:
            if self.args.visual_compared:
                if os.path.splitext(str(self.images[idx]))[0] in self.query_ref:
                    if self.root is not None:
                        img_path = os.path.join(self.root, str(self.images[idx]))
                    else:
                        img_path = str(self.images[idx])
